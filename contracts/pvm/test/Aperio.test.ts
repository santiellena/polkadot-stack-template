...
describe("AperioRepositoryRegistry (PVM)", function () {
	...
	it("creates a repository and sets initial canonical HEAD", async function () {
		...
		const [storedOrganization, storedRepository] = await registry.read.getRepoMetadata([
			repoId,
		]);
	...
	it("accrues pull-based contributor and reviewer rewards only after merge", async function () {
		...
		const treasury = await hre.viem.getContractAt(
			"AperioIncentivesTreasury",
			await registry.read.treasury(),
		);
